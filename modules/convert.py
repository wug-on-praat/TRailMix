...
def convert_trackmalfunctions_to_clingo(malfs, timestep) -> str:
    #mapping = {RailEnvActions.MOVE_FORWARD:"move_forward", RailEnvActions.MOVE_RIGHT:"move_right", RailEnvActions.MOVE_LEFT:"move_left", RailEnvActions.STOP_MOVING:"wait"}
    facts = []
    for m in malfs:
...
def convert_future_positions_to_clingo(positions, timestep) -> str: # TODO: adjust the naming
    facts = []
    # change from list into facts
    future_positions = [clingo_arg for clingo_arg in positions if clingo_arg[2] > timestep+1]
    ...
    for agent, pos, time, dir in current_positions:
        facts.append(f'current_position({agent},{pos},{time},{dir}).\n') #remove: can this be a list of strings or should it be one long string?
