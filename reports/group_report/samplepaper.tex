...
The Secondary Encoding contains three optimization statements (see Code \ref{optimize_code}). In Experiment 3, we seek to test if prioritizing time can still lead to a more efficient rescheduling than Redo Planning. The Secondary Encoding that prioritizes time differs from the Secondary Encoding + heuristic in such that it switches the weights for the two top-most optimization statements. In the time prioritization the weights are as shown in Code \ref{optimize_code}. 
...
\subsection{Base Encoding vs. Secondary Encoding with Time Prioritization}

We compared these two encodings because these are the only encoding that can output the most optimized reschedule 
after a malfunciton.
The base encoding is sometimes faster when there is no consequence,
whereas the secondary encoding with time prioritization is always faster when there
is a consequence.
...
\subsection{Secondary encoding: Time vs. first plan prioritization}

Secondary encoding that prioritizes the first plan is always faster than the secondary encoding that prioritizes time optimization.
...
\begin{itemize}
	\item add summary of results for each experiment
	\item How do we feel about the results?
\end{itemize}
