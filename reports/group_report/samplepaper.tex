...
%
\begin{abstract}
Malfunctions on and around tracks of a railway system is a common real world situation. While train malfunctions render the agent immovable, issues on the track often allows a small window for new decisions and rerouting of agents. We propose an implementation of track malfunctions for the Flatland framework leveraging environment solving with answer set programming. It investigates whether a secondary encoding for rescheduling of agents after a track malfunction is preferable to Redo Planning. 
...
Gilkra2025a have used these features to benchmark different attempts for secondary encodings after train malfunctions \cite{gilkra2025}. They have been able reuse calculated plans from the scheduling step and thus reduce computing time during rescheduling.
...
\subsubsection{Track malfunction Log}
...
If a track malfunction triggers before all trains of the environment have been initialized in their starting position, they have to be positioned during rescheduling. Then the replanning of trains starts. To identify whether an agent is affected by a track malfunction, it raises a \verb|consequence\3| (see Code \ref{consequence}). That means the train is planned to be on the malfunctioning track, it cannot continue on its original path and must adjust its behaviour. There are three scenarios our secondary encoding handles\footnote{If a track malfunction hits a cell while there is a train on its track, the simulation break, effectively corresponding to an accident on the track. We discovered this behaviour and decided to keep it instead of handling that exception as it also depicts a realistic scenario and could be inspiration for future work in this course.}: a) the track malfunction doesn't interfere with the planned routes of agents (no consequence), b) one train is affected by the track malfunction (simple consequence) and c) multiple trains are affected by the track malfunction (complex consequence).
...
Experiment 2 compares performance of the secondary encoding with and without the heuristic prioritizing planned trains.
Experiment 3 evaluates if the base or the secondary encoding with time prioritization is better for getting the time optimized solution. 
...
\section{Results}
The raw results are listed in Appendix \ref{sec:appendix}. Tables \ref{tab:rescheduling_times_base}, \ref{tab:rescheduling_times_heuristic} and \ref{tab:rescheduling_time_priority} report the results by experiment.

\subsection{Base Encoding vs. Secondary Encoding with Heuristics}
For smaller environments, the running time is low for both encodings.
For larger environments, the secondary encoding with heuristics is always preferable.
...
\subsection{Secondary Encoding: Heuristic vs. no Heuristic}

Secondary encoding with heuristic is always faster than secondary encoding without heuristic.

In the beginning, we implemented the secondary encoding without the heuristic
addition (Line 6 in Code \ref{heuristic_code}). We were perplexed by the fact that running the environment with
...
We assume that since the secondary encoding has more rules and options,
it is harder to solve without redirecting Clingo toward the planned train solution.

% Heuristic Flag in Clingo Code
\begin{lstlisting}[style=mystyle, language=Clingo, caption={heuristic code.}, label=heuristic_code]
