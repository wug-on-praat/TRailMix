...
A complex consequence includes multiple agents that require rescheduling (multiple \verb|conequence/3| atoms). However, it also covers the case where the adjustment of one agent's path influences another train, e.g. the rerouted or waiting train would cause a collision with an unaffected train. The trains that were planned as passing through the track malfunction are marked with \verb|consequence/3|. Those agents and the ones indirectly affected are marked as \verb|calculate/1|. The rescheduling then is done as described in Section \ref{simple_consequence}.


\section{Experiments}
We have conducted three experiments to test the capabilities of our encodings in multiple environments. We tested the following scenarios:
\begin{itemize}
	\item Rescheduling: base encoding vs. secondary encoding
...
base prioritzese time, while secondary prioritizes planned trains

Experiment 2 compares performance of the secondary encoding with and without the heuristic prioritizing planned trains. 

Experiment 3 evaluates the weighting of optimization statements in the secondary encoding. optimization weights were switched
...
\subsection{Why is Adding the Heuristic Flag Important?}\label{heuristic}
...
\begin{lstlisting}[style=mystyle, language=Clingo, caption={Here is a caption.}, label=heuristic_code]
...
\section{Future Work}
Future work could consider other approaches than randomness as a decider for a track malfunction. For instance, making malfunction on highly frequented tracks or crossings more likely than on straight connections. 

Another avenue to explore would be the option of a tertiary encoding. The results show that our secondary encoding performs badly when the track malfunction happens early on in an environment with many time steps. The encoding then has to recalculate a very big part and Redo Planning would be more efficient. Introducing a cut-off point for early malfunctions in the secondary encoding that then leads to rescheduling with the base encoding anew could be feasible.

add idea: run just secondary encoding
